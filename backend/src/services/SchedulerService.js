...
class SchedulerService {
  constructor(db, streamService, wsService) {
    ...
    this._runningScheduleIds = new Set(); // lock per schedule ID, bukan global
    this._timer = null;
    this.start();
  }
  ...
  async checkSchedules() {
    try {
      ...
      // Jalankan semua schedule secara parallel, masing-masing punya lock sendiri
  ...
  async _processSchedule(schedule) {
    // Lock per schedule ID — cegah schedule yang sama diproses 2x
    ...
    try {
      // Double-check status di DB dengan row lock
      ...
      if (!fresh.length || fresh[0].status !== 'pending') {
        console.log(`[Scheduler] Schedule ${schedule.id} status bukan pending lagi, skip.`);
        return;
      }
      ...
      this._runningScheduleIds.delete(schedule.id); // selalu cleanup
    }
  }
