    ...
    <div onClick={toggle} className="fixed bottom-0 left-[220px] right-0 bg-[#050508]/90 backdrop-blur-3xl border-t border-white/10 px-6 py-4 flex items-center gap-4 z-[90] cursor-pointer hover:bg-white/5 transition-colors">
      ...
      <div className="flex-1 max-w-md flex flex-col gap-1 z-10">
        <div className="flex justify-between items-center">
          ...
          <div className="text-[10px] text-white/50 font-medium tabular-nums tracking-wider">
...
function ThumbnailTab({ toast }: { toast: (msg: string, type: ToastItem["type"]) => void }) {
  ...
  const handleFileSelect = (files: File[]) => {
    const validFiles = files.filter(file => {
      if (!file.type.startsWith('image/')) { toast(`Hanya JPG, PNG, WebP (${file.name})`, 'error'); return false; }
  ...
  const removePreview = (e: React.MouseEvent, index: number) => {
    e.stopPropagation();
            ...
            <div onDragOver={e=>{e.preventDefault();setDragOver(true)}} onDragLeave={()=>setDragOver(false)} onDrop={handleDrop} onClick={()=>fileInputRef.current?.click()} className={`border-2 border-dashed rounded-2xl p-4 text-center cursor-pointer transition-all overflow-hidden ${dragOver?'border-white/40 bg-white/5':previewFiles.length?'border-transparent bg-white/10':'border-white/10 hover:bg-white/[0.02]'}`}>
              ...
              {previewFiles.length > 0 ? (
                <div className="w-full overflow-x-auto pb-2 flex gap-3 snap-x">
                  {previewFiles.map((p, idx) => (
                    <div key={idx} className="relative group snap-center flex-shrink-0">
                      ...
                      <button onClick={(e) => removePreview(e, idx)} className="absolute -top-2 -right-2 bg-red-500 rounded-full w-5 h-5 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg z-10">
                         <Icon.X />
                      </button>
                      <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-md px-1 py-0.5 text-[8px] text-white truncate rounded-b-lg text-center">
                        {p.file.name}
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex flex-col items-center gap-1 py-4">
                  ...
                  <div className="text-[10px] text-emerald-400 font-medium">Bisa pilih BANYAK file sekaligus</div>
                </div>
              )}
            </div>

            {previewFiles.length > 0 && uploadState !== 'done' && (
              <button onClick={handleUpload} disabled={uploadState==='uploading'} className="w-full mt-3 bg-white text-black py-2.5 rounded-xl text-xs font-bold hover:scale-95 transition-transform disabled:opacity-50 flex items-center justify-center gap-2">
                {uploadState==='uploading' ? '⏳ Mengupload...' : <><Icon.Upload /> Upload {previewFiles.length} File ke VPS</>}
              </button>
            )}
          </div>
        </div>
...
function VideoJadiTab({ toast, queue, onUpload, refresh }: { toast: (msg: string, type: ToastItem["type"]) => void; queue: UploadQueueItem[]; onUpload: (f: File[], c: string) => void; refresh: number }) {
  ...
  const handleUpload = (files: File[]) => {
    ...
    onUpload(files, uploadCat || 'Uncategorized');
  };
      ...
      <div className="flex-1 p-4 md:p-6 flex flex-col gap-6">
        <div className="glass-card-strong rounded-[24px] p-5">
          ...
          <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
            onDrop={e => { e.preventDefault(); setDragOver(false); const f = e.dataTransfer.files; if (f?.length) handleUpload(Array.from(f)); }}
            ...
            <input ref={fileInputRef} type="file" multiple accept=".mp4,.mkv,.mov,.avi,.webm" className="hidden"
              onChange={e => { const f = e.target.files; if (f?.length) handleUpload(Array.from(f)); e.target.value = ''; }} />
            <div className="text-white/30 mb-1 flex justify-center"><Icon.Video /></div>
            <div className="text-[11px] text-white/60 font-medium">{uploading ? '⏳ Uploading...' : 'Drop file video atau klik untuk pilih BANYAK file'}</div>
            <div className="text-[10px] text-white/30 mt-1">MP4 · MKV · MOV · WebM (Bisa upload langsung banyak)</div>
          </div>
        </div>
